...
public class QuanLyBangGiaVung : ControllerBase
{
    ...
    [FromQuery] bool? isActive = true)
    {
        ...
        {
            // 1. Chỉ khởi tạo query, chưa Include vội
            ...
            if (!string.IsNullOrWhiteSpace(searchTerm))
            {
                ...
                // Nếu dùng StartWith sẽ tận dụng Index tốt hơn rất nhiều so với Contains
            ...
            // --- ĐIỂM SỬA QUAN TRỌNG: Sắp xếp trước khi thực thi ---
            ...
            // 4. Lấy dữ liệu (Chỉ Include và Select ở đây)
            var data = await query
                ...
                .Select(bg => new BangGiaVungModels
                {
                    ...
                    TenLoaiHang = bg.MaLoaiHangNavigation.TenLoaiHang
                })
                .ToListAsync();

            result = new { TotalItems = totalItems, TotalPages = totalPages, CurrentPage = page, PageSize = pageSize, Items = data };
    ...
    public async Task<IActionResult> ThemMoi([FromBody] BangGiaVungCreate model)
    {
        if (model == null) return BadRequest(new { message = "Dữ liệu không được để trống." });

        // 1. Validation logic theo loại hình tính giá
        if (model.LoaiTinhGia == 2) // Tính theo Km
        {
            if (model.DonGiaKm <= 0)
                return BadRequest(new { message = "Vận chuyển theo Km bắt buộc đơn giá mỗi Km > 0." });
            if (model.KmToiThieu == null || model.KmToiThieu < 0)
                return BadRequest(new { message = "Km tối thiểu không hợp lệ." });
        }
        else if (model.LoaiTinhGia == 1) // Tính theo Vùng/Kiện
        {
            if (model.TrongLuongToiDaKg <= 0)
                return BadRequest(new { message = "Trọng lượng tối đa phải lớn hơn 0." });
            if (model.DonGiaCoBan <= 0)
                return BadRequest(new { message = "Đơn giá cơ bản phải lớn hơn 0." });
        }
        else
        {
            return BadRequest(new { message = "Loại hình tính giá không hợp lệ (1: Vùng, 2: Km)." });
        }

        // 2. Kiểm tra trùng lặp (Logic: Một tuyến đường + Một loại hàng + Một cách tính chỉ nên có 1 bảng giá Active)
        // Đã bỏ UNIQUE KEY ở DB nên ta kiểm tra bằng code để đảm bảo tính nghiệp vụ
        bool isExist = await _context.BangGiaVungs.AnyAsync(bg =>
            bg.IsActive == true &&
        ...
        if (isExist)
        {
            return BadRequest(new { message = "Đã tồn tại bảng giá đang hoạt động cho tuyến đường và loại hàng này." });
        }

        try
        {
            // 3. Mapping dữ liệu từ DTO sang Entity thực tế
            var newEntry = new BangGiaVung
            {
                KhuVucLay = model.KhuVucLay,
                KhuVucGiao = model.KhuVucGiao,
                TrongLuongToiThieuKg = model.TrongLuongToiThieuKg,
                TrongLuongToiDaKg = model.TrongLuongToiDaKg,
                DonGiaCoBan = model.DonGiaCoBan,
                PhuPhiMoiKg = model.PhuPhiMoiKg,
                LoaiTinhGia = model.LoaiTinhGia,
                DonGiaKm = model.DonGiaKm,
                PhiDungDiem = model.PhiDungDiem,
                KmToiThieu = model.KmToiThieu,
            ...
            _context.BangGiaVungs.Add(newEntry);
            await _context.SaveChangesAsync();

            // 4. Ghi log (Sử dụng ID đã sinh ra từ DB: newEntry.MaBangGia)
            ...
            return Ok(new { message = "Thêm bảng giá thành công", maBangGia = newEntry.MaBangGia });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Lỗi thêm bảng giá");
            return StatusCode(500, new { message = "Lỗi hệ thống", detail = ex.InnerException?.Message ?? ex.Message });
        }
    }
    ...
    public async Task<IActionResult> PhanTichDichVu([FromBody] YeuCauTinhPhi request)
    {
        try
        {
            ...
            // Ngưỡng xác định hàng nặng
            bool laHangNang = trongLuongDeTinh > 1000;

            // --- 2. XÁC ĐỊNH NHÃN TUYẾN ĐƯỜNG (CHỈ DỰA VÀO TỈNH) ---
            string nhanLay = "";
            string nhanGiao = "";

            if (request.ThanhPhoLay.Trim().Equals(request.ThanhPhoGiao.Trim(), StringComparison.OrdinalIgnoreCase))
            {
                // Trường hợp NỘI TỈNH: Cả nơi lấy và giao đều dùng nhãn NOI_TINH
                nhanLay = "NOI_TINH";
                nhanGiao = "NOI_TINH";
            }
            else
            {
                // Trường hợp LIÊN TỈNH: Lấy nhãn theo Miền (Bắc, Trung, Nam)
            ...
            // --- 4. LỌC THEO TRỌNG LƯỢNG VÀ LOẠI HÀNG ---
            var danhSachLoc = tatCaBangGia
                .Where(bg =>
                {
                    ...
                    // Lọc theo trọng lượng cho hàng nặng (nguyên chuyến)
                    if (laHangNang)
                    {
                        return bg.LoaiTinhGia == 2 || (bg.LoaiTinhGia == 1 && bg.TrongLuongToiDaKg > 1000);
                    }

                    // Tìm trọng lượng tối đa của tuyến này (để xử lý hàng vượt mốc)
            ...
            var ketQua = danhSachLoc.Select(bg =>
            {
                decimal tongTien = 0;
                string chiTietGia = "";
                string tenDichVu = "";
